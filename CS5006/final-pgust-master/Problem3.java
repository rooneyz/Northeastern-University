...
final public class Problem3 {
	...
	public static int findCounterfeit(String coins) {
		...
		// loop if two or more coins to compare
		while (right-left > 1) {
			// exclude last coin if odd number
			right -= (right-left) % 2;  

			// compare the two piles of coins
			int mid = left + (right-left)/2;
			int c = coins.substring(left,mid).compareTo(coins.substring(mid,right));
			
			if (c == 0) {  // both piles same: counterfeit is odd last coin
				left = right; 
			} else if (c < 0) {  // counterfeit in lower half
				right = mid;
			} else {  // counterfeit in upper half
				left = mid;  
			}
		}
		return left;
