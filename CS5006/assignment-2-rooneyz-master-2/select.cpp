...
int select0(int a[], int lo, int hi, int k) {
	// Create local variable of low and hi
	int low = lo;
	int high = hi;

	// Step 1 invert base conditions
	// Step 2 eliminate recursion by looping
	while (!(low > high)) {
		// Partitioning index
		...
		if (k == p) {
			...
			//decreament high instead of doing it in call to select0(a, lo, p - 1, k);
			high -= 1;
		} else {
			//increment lo instead of doing it in call to select0(a, p + 1, high, k);
			low += 1; //return select0(a, p + 1, hi, k);
		}
...
void test_select() {
	...
	// repeated value
	int arrayR[] = {1, 0, 3, 4, 6, 3};
	actual = select(arrayR, 6, 2);
	CU_ASSERT_EQUAL(actual, 3);

	// other repeated value
	int arrayR1[] = {1, 0, 3, 4, 6, 3};
	actual = select(arrayR1, 6, 3);
	CU_ASSERT_EQUAL(actual, 3);

	// sorted array
	int arrayS[] = {0, 1, 2, 3, 4, 5, 6};
	actual = select(arrayS, 7, 3);
	CU_ASSERT_EQUAL(actual, 3);

	// reverse sorted array
	int arrayRev[] = {6, 5, 4, 3, 2, 1, 0};
	actual = select(arrayRev, 7, 3);
	CU_ASSERT_EQUAL(actual, 3);
