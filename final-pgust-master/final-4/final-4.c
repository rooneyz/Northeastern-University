...
void borrowBook(Borrower *borrower) {
	...
	// can only checked out library if before closing time
	if (borrower->current_time < CLOSING_TIME) {
		// borrow a book
		pthread_mutex_lock(&library_mutex);
			int book = recommendBook();
			if (book == NO_BOOK) {
				sprintf(msg, "no book to borrow");
			} else if (setBookState(book, borrowed)) {
				borrower->book = book;
				borrower->due_by = borrower->current_time + CIRCULATION_PERIOD;
				if (borrower->due_by > CLOSING_TIME) {
					borrower->due_by = CLOSING_TIME;
				}
				borrower->borrowed_count++;
				sprintf(msg, "borrowed book %d", book);
			} else {
				sprintf(msg, "***unavailable: %d", book);
			}
			report(borrower, msg);
...
void returnBook(Borrower *borrower) {
	...
	// return book if due
	pthread_mutex_lock(&library_mutex);
		if (setBookState(borrower->book, checked_in)) {
			sprintf(msg, "returned book %d", borrower->book);
		} else {
			sprintf(msg, "***already returned %d", borrower->book);
		}
		report(borrower, msg);
	pthread_mutex_unlock(&library_mutex);
